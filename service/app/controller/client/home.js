...
class HomeController extends Controller {
    ...
    async getListById() {
        const id = this.ctx.params.id
        const sql = `select article.id as id,article.title as title,article.introduce as introduce,article.content as content,article.view_count as view_count,type.typeName as typeName,FROM_UNIXTIME(article.create_at,'%Y-%m-%d %H:%i:%s') as create_at from article left join type on article.type_id = type.id where article.type_id=${id}`
        const result = await this.app.mysql.query(sql)
        this.ctx.body = {
            data: result
        }
